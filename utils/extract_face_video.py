...
def similarity_transform(img, pts):
    # convert x1,x2..xn,y1,y2..,yn to x1,y1,x2,y1..,xn,yn
    def pt2xy(pt):
        num = int(len(point) / 2)
        set_x = pt[:5]
        set_y = pt[5:]
        set_xy = []
        for i in range(num):
            set_xy.append(set_x[i])
            set_xy.append(set_y[i])
        return set_xy

    src_set = np.array(pt2xy(point), dtype=np.int8)
    src_set = np.reshape(src_set, [1, 5, 2])
    #        cv2.circle(img, (src_set[0], src_set[1]), 1, [255, 255, 0])
    trans = cv2.estimateRigidTransform(src_set, fixed_set, fullAffine=False)
    cv2.warpAffine(img, trans, (CROPPED_WIDTH, CROPPED_HEIGHT))
...
#video_listfile = "video_list.txt"
out_path = '/home/ubuser/swei/data_and_backup/dataset/YawDD_cropped/Mirror/Male/Yawning/'
#out_path = '/home/ubuser/swei/data_and_backup/dataset/YawDD_cropped/Dash/Female/'
NUM_FRAMES = 10
CROPPED_WIDTH = 224
CROPPED_HEIGHT = 224

fixed_set = [56,67,156,67,113,122,76,162,154,160]
fixed_set = np.array(fixed_set, dtype=np.int32)
fixed_set = np.reshape(fixed_set, [1, 5, 2])

def process(video_listfile):
    minsize = 20
    ...
    #error = []
    f = open(video_listfile, 'r')
    for video_path in f.readlines():
        tic()

        video_path = video_path.split('\n')[0]
        video_name = video_path.split('/')[-1]
        print(video_path)
        if os.path.exists(os.path.join(out_path, video_name)):
            continue
        writer = cv2.VideoWriter(os.path.join(out_path, video_name), fourcc= cv2.VideoWriter_fourcc('X', 'V', 'I', 'D'),
                                 fps=25, frameSize=(CROPPED_WIDTH, CROPPED_HEIGHT))
        ...
        while success:
            ...
            if len(boundingboxes) == 0:
                ...
                if len(pre_nframe_bboxes) == 0:
                    ...
                    point = np.average(pre_nframe_points, 0)
                ...
                point = points[idx[0], :]
            if len(pre_nframe_bboxes) >= NUM_FRAMES:
                pre_nframe_bboxes.pop(0)
                pre_nframe_points.pop(0)
            pre_nframe_bboxes.append(bbox)
            pre_nframe_points.append(point)
            ...
            if FACE_ALIGN:
                # convert x1,x2..xn,y1,y2..,yn to x1,y1,x2,y1..,xn,yn
                def pt2xy(pt, pt_coord):
                    num = int(len(point) / 2)
                    set_x = pt[:5] - pt_coord[0]
                    set_y = pt[5:] - pt_coord[1]
                    set_xy = []
                    for i in range(num):
                        set_xy.append(set_x[i])
                        set_xy.append(set_y[i])
                ...
                src_set = np.array(pt2xy(point, [bbox[0], bbox[1]]), dtype=np.int32)
                src_set = np.reshape(src_set, [1, 5, 2])

                trans = cv2.estimateRigidTransform(src_set, fixed_set, fullAffine=True)

                face_trans = cv2.warpAffine(face, trans, (CROPPED_WIDTH, CROPPED_HEIGHT))

            face_trans = cv2.resize(face_trans, (CROPPED_WIDTH, CROPPED_HEIGHT))
            writer.write(face_trans)

            if DEBUG:
                cv2.imshow('face', face)
                cv2.imshow('face_trans', face_trans)
                img = drawBoxes(img, bbox)
                for i in range(5):
                    cv2.circle(img, (point[i], point[5+i]), 2, [0, 0, 255])
                cv2.imshow('img', img)
                ch = cv2.waitKey(40) & 0xFF
                if ch == 27:
                    break

            success, img = cap.read()
            fcnt += 1
            print('Processing %d / %d' % (fcnt, ftotal))
        cap.release()
        writer.release()
