...
fixed_set = np.array(fixed_set, dtype=np.float32)
...
def main():
    ...
    for imgpath in f.readlines():
        ...
        face = cv2.resize(face, (CROPPED_WIDTH, CROPPED_HEIGHT))
        ...
        def pt2xy(pt, pt_coord):
            ...
            return set_xy
        src_set = np.array(pt2xy(point, [bbox[0], bbox[1]]), dtype=np.float32)
...
#        cv2.circle(img, (src_set[0], src_set[1]), 1, [255, 255, 0])
        ...
        face_new = cv2.warpAffine(face, trans, (CROPPED_WIDTH, CROPPED_HEIGHT))
        print(face_new)
        cv2.imshow('face_trans', face_new)
